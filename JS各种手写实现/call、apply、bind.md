...
1. 被 bind 绑定之后的绑定函数，再使用bind绑定时，传入的 thisArg 无效，但是之前传递的参数依然有效（这里的保留参数是不是有点像**柯里化**函数）
...
使用 new 构造被bind绑定的函数时，bind提供的 this 值会被忽略，参数会被正常传递执行。

从上一章的内容（手写 new 的实现）我们可知，当执行 new Function 这种构造写法的时候，new 的内部会以 Function 为原型新创建一个对象，并通过 apply(thisArg, [arg]) 这种形式绑定到执行 Function 函数的 this上下文。
